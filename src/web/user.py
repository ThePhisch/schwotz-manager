...
def user_api(usecase: UserUsecases):
    ...
    async def delete_user(task_id: int, usecase: UserUsecases = Depends(get_usecase)):
        usecase.delete_user(task_id)
        return {"message": "User deleted"}

    @route.post("/")
    async def add_user(task: User, usecase: UserUsecases = Depends(get_usecase)):
        new_user_id = usecase.add_user(task)
